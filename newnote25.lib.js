...
window.handleAdd=async()=>{if(!currentNote||!noteTextarea)return;const r=await showModal({header:`<div class="modal-title">Add Text to Lines</div>`,body:`<div style="display:flex;flex-direction:column;gap:10px;"><div style="display:flex;flex-direction:column;gap:6px;"><label class="modal-label">Insert text</label><input type="text" id="insertText" placeholder="Text to insert (use %L for line number, %N for new line)" class="modal-input"></div><div style="display:flex;flex-direction:column;gap:6px;"><label class="modal-label">Insert position</label><div class="custom-dropdown"><div id="insertPosition" class="custom-dropdown-trigger modal-input" data-options='[{"label":"Insert at start of line","value":"start"},{"label":"Insert at end of line","value":"end"},{"label":"Insert at specific column","value":"column"}]'>Insert at start of line</div></div></div><div style="display:flex;flex-direction:column;gap:6px;"><label class="modal-label">Column number</label><input type="number" id="insertColumn" min="1" placeholder="Column number (1-based)" class="modal-input" style="display:none"></div></div>`,footer:`<button onclick="closeModal()">Cancel</button><button onclick="handleAddSubmit()" data-skip-validation style="background: var(--accent-2); color: #cacaca;">Add</button>`});if(!r||r.action!=="Add")return;const posTrigger=modalScope.insertPosition,colInput=modalScope.insertColumn;posTrigger.addEventListener("change",()=>{colInput.style.display=posTrigger.dataset.value==="column"?"block":"none"})};window.handleAddSubmit=()=>{const insert=modalScope.insertText?modalScope.insertText.value:"",pos=modalScope.insertPosition?modalScope.insertPosition.dataset.value:"start",col=parseInt(modalScope.insertColumn?modalScope.insertColumn.value:1,10)||1;closeModal({action:"Add",insert:insert,position:pos,column:col})};window.applyAddToLines=(insert,pos,col)=>{if(!noteTextarea)return;const lines=noteTextarea.value.split('\n');let res="";for(let i=0;i<lines.length;i++){let line=lines[i];const rep=insert.replace(/%L/g,String(i+1)).replace(/%N/g,"\n");if(pos==="start")line=rep+line;else if(pos==="end")line=line+rep;else if(pos==="column"){const idx=Math.max(0,col-1);if(line.length<idx)line=line.padEnd(idx," ");line=line.slice(0,idx)+rep+line.slice(idx)}res+=line+(i<lines.length-1?"\n":"")}noteTextarea.value=res;if(typeof updateNoteMetadata==="function")updateNoteMetadata();if(typeof updatecounts==="function")updatecounts();showNotification("Text added successfully!")};    
