...
class L{
  ...
  buildPrimaryExtensionMap(){
    for(const[ext,lang]of Object.entries(this.extMap)){
      if(!this.primary[lang]){
        this.primary[lang]=ext;
      }
    }
  }
  ...
  async load(lang){
    ...
    const p=this._loadWithDeps(lang);
  ...
  async _loadWithDeps(lang){
    ...
    for(const dep of deps){
      await this._loadSingle(dep);
    }
    await this._loadSingle(lang);
  }
  async _loadSingle(lang){
    ...
    return new Promise((res,rej)=>{
      ...
      s.onerror=()=>{this.loading.delete(lang);rej(new Error(`load failed: ${lang}`))};
