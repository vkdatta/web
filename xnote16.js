  ...
  window.populateNoteList();
        ...
        const result = await showModal({
          ...
          body: `
            ...
            <div class="custom-dropdown">
              <div id="formatType" class="custom-dropdown-trigger modal-input" data-options='${JSON.stringify(
                optionsArray
              )}'>Select option</div>
            </div>
          `,
      ...
      return preserveSelection(async function () {
        ...
        const result = await showModal({
          ...
          body: `
            <label class="modal-label">Choose Format</label>
            <div class="custom-dropdown">
              <div id="listFormat" class="custom-dropdown-trigger modal-input" data-options='${JSON.stringify(
                optionsArray
              )}'>Select option</div>
            </div>
          `,
          ...
          body: `
            ...
            <div style="display: flex; gap: 10px; align-items: center;">
              <div style="flex: 1;">
                ...
                <div class="custom-dropdown">
                  <div id="linkFormat" class="custom-dropdown-trigger modal-input" data-options='${JSON.stringify(
                    formatOptionsArray
                  )}'>Select option</div>
                </div>
              </div>
          ...
          body: `
            ...
            <div style="display: flex; gap: 10px; align-items: center;">
              <div style="flex: 1;">
                <label class="modal-label">Format</label>
                <div class="custom-dropdown">
                  <div id="imageFormat" class="custom-dropdown-trigger modal-input" data-options='${JSON.stringify(
                    formatOptionsArray
                  )}'>Select option</div>
                </div>
              </div>
    ...
    window.handleCopyNote = function () {
      ...
      navigator.clipboard
        ...
        .catch(() =>
          showNotification("Copy failed (clipboard not available).")
        );
    };
    ...
    async function checkPassword(note, callback) {
      const r = await showModal({
        header: `<div class="modal-title">Enter Password</div>`,
        body: `<div><label class="modal-label">Please enter the password for this note:</label><input type="password" id="password" class="modal-input"></div>`,
        footer: `<button onclick="closeModal()">Cancel</button><button onclick="handleCheckPasswordSubmit()" style="background: var(--accent-2); color: #cacaca;">Submit</button>`
      });
      ...
      if (password === note.password) {
        ...
        const r2 = await showModal({
          header: `<div class="modal-title">Password Recovery</div>`,
          body: `<div><label class="modal-label">Enter DOB (YYYY-MM-DD) or clear note:</label><input type="text" id="dobInput" class="modal-input"></div>`,
          footer: `<button onclick="closeModal()">Cancel</button><button onclick="handleRecoverySubmit()" style="background: var(--accent-2); color: #cacaca;">Submit</button>`
        });
        if (!r2 || r2.action !== "submit") return;
        const dobInput = r2.dobInput;
        if (dobInput === dob) {
          callback();
          showNotification("Note unlocked via DOB!");
        } else {
          const r3 = await showModal({
            header: `<div class="modal-title">Clear Note</div>`,
            body: `<div class="modal-label">Incorrect DOB. Clear note to continue?</div>`,
            footer: `<button onclick="closeModal()">No</button><button onclick="handleClearConfirm()" style="background: var(--accent-2); color: #cacaca;">Yes</button>`
          });
          if (r3 && r3.confirmed) {
            note.content = "";
            note.password = "";
            saveNotes();
            callback();
            showNotification("Note cleared and unlocked!");
          }
        }
      }
    }
    window.handleCheckPasswordSubmit = function () {
      closeModal({ action: "submit", password: modalScope.password.value });
    };
    window.handleRecoverySubmit = function () {
      closeModal({ action: "submit", dobInput: modalScope.dobInput.value });
    };
    window.handleClearConfirm = function () {
    ...
    function checkPasswordRequirement() {
      ...
      if (!lastCheck || now - parseInt(lastCheck) > 15 * 60 * 1000) {
        notes.forEach((note) => {
          if (note.password && note === currentNote) {
            checkPassword(note, () => {});
          }
        });
    ...
    window.handlePasswordProtect = async () => {
      if (!currentNote) {
      ...
      if (!currentNote.password) {
        ...
        if (!dob) {
          r = await showModal({
            header: `<div class="modal-title">Set Password</div>`,
            body: `<div><label class="modal-label">Enter password:</label><input type="password" id="password" class="modal-input"></div><div><label class="modal-label">Enter DOB (YYYY-MM-DD) for recovery:</label><input type="text" id="dobInput" class="modal-input"></div>`,
            footer: `<button onclick="closeModal()">Cancel</button><button onclick="handleSetPasswordDobSubmit()" style="background: var(--accent-2); color: #cacaca;">Set</button>`
          });
          if (!r || r.action !== "submit") return;
          const { password, dobInput } = r;
          if (password && dobInput) {
            const r2 = await showModal({
              header: `<div class="modal-title">Security Warning</div>`,
              body: `<div class="modal-label">Password is stored in local storage and may be accessible to tech-savvy users. Proceed?</div>`,
              footer: `<button onclick="closeModal()">No</button><button onclick="handleWarningConfirm()" style="background: var(--accent-2); color: #cacaca;">Yes</button>`
            });
            if (r2 && r2.confirmed) {
              ...
              dob = dobInput;
              saveNotes();
              showNotification("Password set!");
            }
          }
        } else {
          r = await showModal({
            header: `<div class="modal-title">Set Password</div>`,
            body: `<div><label class="modal-label">Enter password:</label><input type="password" id="password" class="modal-input"></div>`,
            footer: `<button onclick="closeModal()">Cancel</button><button onclick="handleSetPasswordSubmit()" style="background: var(--accent-2); color: #cacaca;">Set</button>`
          });
          if (!r || r.action !== "submit") return;
          const password = r.password;
          if (password) {
        ...
        const r = await showModal({
          header: `<div class="modal-title">Password Options</div>`,
          body: `<div><label class="modal-label">Remove or change password? Enter "remove" or "change":</label><input type="text" id="action" class="modal-input"></div>`,
          footer: `<button onclick="closeModal()">Cancel</button><button onclick="handleActionSubmit()" style="background: var(--accent-2); color: #cacaca;">Submit</button>`
        });
        if (!r || r.action !== "submit") return;
        const action = r.action;
        if (action === "remove") {
          const r2 = await showModal({
            header: `<div class="modal-title">Remove Password</div>`,
            body: `<div><label class="modal-label">Enter current password:</label><input type="password" id="password" class="modal-input"></div>`,
            footer: `<button onclick="closeModal()">Cancel</button><button onclick="handleRemovePasswordSubmit()" style="background: var(--accent-2); color: #cacaca;">Remove</button>`
          });
          if (!r2 || r2.action !== "submit") return;
          const password = r2.password;
          ...
          const r2 = await showModal({
            header: `<div class="modal-title">Change Password</div>`,
            body: `<div><label class="modal-label">Enter current password:</label><input type="password" id="oldPassword" class="modal-input"></div><div><label class="modal-label">Enter new password:</label><input type="password" id="newPassword" class="modal-input"></div>`,
            footer: `<button onclick="closeModal()">Cancel</button><button onclick="handleChangePasswordSubmit()" style="background: var(--accent-2); color: #cacaca;">Change</button>`
          });
          if (!r2 || r2.action !== "submit") return;
          const { oldPassword, newPassword } = r2;
          if (oldPassword === currentNote.password) {
            currentNote.password = newPassword;
            saveNotes();
    ...
    window.handleSetPasswordDobSubmit = function () {
      closeModal({
        action: "submit",
        password: modalScope.password.value,
        dobInput: modalScope.dobInput.value
      });
    };
    window.handleSetPasswordSubmit = function () {
      closeModal({ action: "submit", password: modalScope.password.value });
    };
    window.handleWarningConfirm = function () {
      closeModal({ confirmed: true });
    };
    window.handleActionSubmit = function () {
      closeModal({ action: "submit", action: modalScope.action.value });
    };
    window.handleRemovePasswordSubmit = function () {
      closeModal({ action: "submit", password: modalScope.password.value });
    };
    window.handleChangePasswordSubmit = function () {
      closeModal({
        action: "submit",
        oldPassword: modalScope.oldPassword.value,
        newPassword: modalScope.newPassword.value
      });
    };
    (window.handleRename = function () {
      const e = window.preserveSelection(async function () {
        if (!window.currentNote)
          return void window.showNotification("No note selected");
        const t = window.currentNote.title || "",
          n = window.currentNote.extension || "",
          o = await showModal({
            ...
            footer:
              '<button onclick="closeModal()">Cancel</button><button onclick="handleRenameSubmit()" style="background:var(--accent-2);color:#052027;">OK</button>'
          });
        ...
        (a = a || t),
          (i = i || n),
          (window.currentNote.title = a),
          (window.currentNote.extension = i.replace(/^\./, "").toLowerCase()),
          (window.currentNote.lastEdited = new Date().toISOString());
        const l = notes.findIndex((e) => e.id === window.currentNote.id);
        -1 !== l &&
          ((notes[l].title = a),
          (notes[l].extension = window.currentNote.extension),
          (notes[l].lastEdited = window.currentNote.lastEdited)),
      ...
      (window.handleRenameSubmit = function () {
        closeModal({
          ...
          newExtension: modalScope.newExtension
            ? modalScope.newExtension.value
            : ""
        });
      });
    window.handleDownload = async function () {
      ...
      const res = await showModal({
        ...
        footer: `<button onclick="closeModal()">Cancel</button><button onclick="handleDownloadSubmit()" data-skip-validation style="background: var(--accent-2); color: #052027;">Download</button>`
      });
...
var MD5 = function (d) { var r = M(V(Y(X(d), 8 * d.length))); return r.toLowerCase(); }; function M(d) { for (var _, m = "0123456789ABCDEF", f = "", r = 0; r < d.length; r++) (_ = d.charCodeAt(r)), (f += m.charAt((_ >>> 4) & 15) + m.charAt(15 & _)); return f; } function X(d) { for (var _ = Array(d.length >> 2), m = 0; m < _.length; m++) _[m] = 0; for (m = 0; m < 8 * d.length; m += 8) _[m >> 5] |= (255 & d.charCodeAt(m / 8)) << m % 32; return _; } function V(d) { for (var _ = "", m = 0; m < 32 * d.length; m += 8) _ += String.fromCharCode((d[m >> 5] >>> m % 32) & 255); return _; } function Y(d, _) { (d[_ >> 5] |= 128 << _ % 32), (d[14 + (((_ + 64) >>> 9) << 4)] = _); for ( var m = 1732584193, f = -271733879, r = -1732584194, i = 271733878, n = 0; n < d.length; n += 16 ) { var h = m, t = f, g = r, e = i; (f = md5_ii( (f = md5_ii( (f = md5_ii( (f = md5_ii( (f = md5_hh( (f = md5_hh( (f = md5_hh( (f = md5_hh( (f = md5_gg( (f = md5_gg( (f = md5_gg( (f = md5_gg( (f = md5_ff( (f = md5_ff( (f = md5_ff( (f = md5_ff( f, (r = md5_ff( r, (i = md5_ff( i, (m = md5_ff( m, f, r, i, d[n + 0], 7, -680876936 )), f, r, d[n + 1], 12, -389564586 )), m, f, d[n + 2], 17, 606105819 )), i, m, d[n + 3], 22, -1044525330 )), (r = md5_ff( r, (i = md5_ff( i, (m = md5_ff( m, f, r, i, d[n + 4], 7, -176418897 )), f, r, d[n + 5], 12, 1200080426 )), m, f, d[n + 6], 17, -1473231341 )), i, m, d[n + 7], 22, -45705983 )), (r = md5_ff( r, (i = md5_ff( i, (m = md5_ff( m, f, r, i, d[n + 8], 7, 1770035416 )), f, r, d[n + 9], 12, -1958414417 )), m, f, d[n + 10], 17, -42063 )), i, m, d[n + 11], 22, -1990404162 )), (r = md5_ff( r, (i = md5_ff( i, (m = md5_ff( m, f, r, i, d[n + 12], 7, 1804603682 )), f, r, d[n + 13], 12, -40341101 )), m, f, d[n + 14], 17, -1502002290 )), i, m, d[n + 15], 22, 1236535329 )), (r = md5_gg( r, (i = md5_gg( i, (m = md5_gg( m, f, r, i, d[n + 1], 5, -165796510 )), f, r, d[n + 6], 9, -1069501632 )), m, f, d[n + 11], 14, 643717713 )), i, m, d[n + 0], 20, -373897302 )), (r = md5_gg( r, (i = md5_gg( i, (m = md5_gg( m, f, r, i, d[n + 5], 5, -701558691 )), f, r, d[n + 10], 9, 38016083 )), m, f, d[n + 15], 14, -660478335 )), i, m, d[n + 4], 20, -405537848 )), (r = md5_gg( r, (i = md5_gg( i, (m = md5_gg( m, f, r, i, d[n + 9], 5, 568446438 )), f, r, d[n + 14], 9, -1019803690 )), m, f, d[n + 3], 14, -187363961 )), i, m, d[n + 8], 20, 1163531501 )), (r = md5_gg( r, (i = md5_gg( i, (m = md5_gg( m, f, r, i, d[n + 13], 5, -1444681467 )), f, r, d[n + 2], 9, -51403784 )), m, f, d[n + 7], 14, 1735328473 )), i, m, d[n + 12], 20, -1926607734 )), (r = md5_hh( r, (i = md5_hh( i, (m = md5_hh(m, f, r, i, d[n + 5], 4, -378558)), f, r, d[n + 8], 11, -2022574463 )), m, f, d[n + 11], 16, 1839030562 )), i, m, d[n + 14], 23, -35309556 )), (r = md5_hh( r, (i = md5_hh( i, (m = md5_hh(m, f, r, i, d[n + 1], 4, -1530992060)), f, r, d[n + 4], 11, 1272893353 )), m, f, d[n + 7], 16, -155497632 )), i, m, d[n + 10], 23, -1094730640 )), (r = md5_hh( r, (i = md5_hh( i, (m = md5_hh(m, f, r, i, d[n + 13], 4, 681279174)), f, r, d[n + 0], 11, -358537222 )), m, f, d[n + 3], 16, -722521979 )), i, m, d[n + 6], 23, 76029189 )), (r = md5_hh( r, (i = md5_hh( i, (m = md5_hh(m, f, r, i, d[n + 9], 4, -640364487)), f, r, d[n + 12], 11, -421815835 )), m, f, d[n + 15], 16, 530742520 )), i, m, d[n + 2], 23, -995338651 )), (r = md5_ii( r, (i = md5_ii( i, (m = md5_ii(m, f, r, i, d[n + 0], 6, -198630844)), f, r, d[n + 7], 10, 1126891415 )), m, f, d[n + 14], 15, -1416354905 )), i, m, d[n + 5], 21, -57434055 )), (r = md5_ii( r, (i = md5_ii( i, (m = md5_ii(m, f, r, i, d[n + 12], 6, 1700485571)), f, r, d[n + 3], 10, -1894986606 )), m, f, d[n + 10], 15, -1051523 )), i, m, d[n + 1], 21, -2054922799 )), (r = md5_ii( r, (i = md5_ii( i, (m = md5_ii(m, f, r, i, d[n + 8], 6, 1873313359)), f, r, d[n + 15], 10, -30611744 )), m, f, d[n + 6], 15, -1560198380 )), i, m, d[n + 13], 21, 1309151649 )), (r = md5_ii( r, (i = md5_ii( i, (m = md5_ii(m, f, r, i, d[n + 4], 6, -145523070)), f, r, d[n + 11], 10, -1120210379 )), m, f, d[n + 2], 15, 718787259 )), i, m, d[n + 9], 21, -343485551 )), (m = safe_add(m, h)), (f = safe_add(f, t)), (r = safe_add(r, g)), (i = safe_add(i, e)); } return Array(m, f, r, i); } function md5_cmn(d, _, m, f, r, i) { return safe_add(bit_rol(safe_add(safe_add(_, d), safe_add(f, i)), r), m); } function md5_ff(d, _, m, f, r, i, n) { return md5_cmn((_ & m) | (~_ & f), d, _, r, i, n); } function md5_gg(d, _, m, f, r, i, n) { return md5_cmn((_ & f) | (m & ~f), d, _, r, i, n); } function md5_hh(d, _, m, f, r, i, n) { return md5_cmn(_ ^ m ^ f, d, _, r, i, n); } function md5_ii(d, _, m, f, r, i, n) { return md5_cmn(m ^ (_ | ~f), d, _, r, i, n); } function safe_add(d, _) { var m = (65535 & d) + (65535 & _); return (((d >> 16) + (_ >> 16) + (m >> 16)) << 16) | (65535 & m); } function bit_rol(d, _) { return (d << _) | (d >>> (32 - _)); }
    ...
    window.capitalizeSentences = preserveSelection(async () => {
      ...
      if (s === e) {
        ...
        const r = n.replace(/(^\s*[a-z])|([.!?]\s*[a-z])/g, (t) =>
          t.toUpperCase()
        );
        noteTextarea.value = t.substring(0, s) + r + t.substring(e);
      }
    ...
    window.handlePattern = async function () {
      ...
      const r = await showModal({
        ...
        footer: `<button onclick="closeModal()">Cancel</button><button onclick="handlePatternSubmit()" style="background: var(--accent-2); color: #052027;">Replace</button>`
      });
      if (!r || r.action !== "submit") return;
      const {
        startDelim,
      ...
      if (mode === "single") {
        ...
        if (isNaN(n) || n < 1)
          return showNotification("Invalid instance number");
      ...
      try {
        ...
        const replacedCount = Math.max(
          0,
          Math.min(to, pairs.length) - from + 1
        );
        showNotification(
    ...
    window.handlePatternSubmit = function () {
      const s = modalScope.startDelim ? modalScope.startDelim.value.trim() : "",
        ...
        inc = modalScope.includeDelims
          ? modalScope.includeDelims.checked
          : false,
      ...
      if (tid === "startDelim" || tid === "endDelim") {
        ...
        if (!s || !f) {
          modalScope.matchInfo &&
            (modalScope.matchInfo.textContent = "Matches: 0");
          return;
        }
        ...
        if (pairs.length) {
          ...
          modalScope.rangeTo &&
            (modalScope.rangeTo.value = String(pairs.length));
    ...
    window.handleAdd = async () => {
      ...
      const r = await showModal({
        ...
        footer: `<button onclick="closeModal()">Cancel</button><button onclick="handleAddSubmit()" style="background: var(--accent-2); color: #cacaca;">Add</button>`,
        html: true
      });
      if (!r || r.action !== "submit") return;
    ...
    window.handleAddSubmit = function () {
      const insertText = modalScope.insertText
        ? modalScope.insertText.value
        : "";
    ...
    window.handleCleanupText = async () => {
      ...
      const r = await showModal({
        ...
        footer: `<button onclick="closeModal()">Cancel</button><button onclick="handleCleanupSubmit()" style="background: var(--accent-2); color: #cacaca;">Cleanup</button>`,
        html: true
      });
      if (!r || r.action !== "submit") return;
    ...
    window.handleCleanupSubmit = function () {
      const cleanupStyle = modalScope.cleanupStyle
        ...
        : "";
      const trimNumber = modalScope.trimNumber
        ? modalScope.trimNumber.value
        : "1";
      const trimSide = modalScope.trimSide
        ? modalScope.trimSide.dataset.value
        : "left";
      if (!cleanupStyle)
        return showNotification("Please select a cleanup style!");
    ...
    document.addEventListener("click", (e) => {
      ...
      if (trimContainer) {
        trimContainer.style.display =
          value === "trim_columns" ? "flex" : "none";
      }
    });
