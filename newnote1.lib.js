function initApp() {
  (function () {
    ...
    function ensureModal() {
      ...
      titleEl.id = "modal_title_" + Math.random().toString(36).slice(2, 8);
    ...
    function trapFocus(enable = true) {
      if (!backdrop) return;
      if (enable) {
    ...
    function createModalElement(type, opts = {}) {
...
if(type==='input'){
  ...
  return out;
      ...
      if (type === "row") {
        ...
        (opts.children || []).forEach((child) => {
          ...
          else if (child.el) container.appendChild(child.el);
        });
        out.el = container;
        return out;
      }
    ...
    function collectValues(state) {
      const values = {};
      if (!state || !state.inputs) return values;
      for (const el of state.inputs) {
        ...
        if (el.type === "checkbox" || el.type === "radio") val = el.checked;
          ...
          el.classList &&
          el.classList.contains("custom-dropdown-trigger")
        )
          val = el.dataset.value || el.textContent;
        else val = el.value;
        values[key] = val;
      }
      return values;
    }
    function setupConditionals(state) {
      state.inputs.forEach((el) => {
        if (el.classList && el.classList.contains("custom-dropdown-trigger"))
          el.addEventListener("change", () => updateVisibility(state));
      });
      updateVisibility(state);
    }
    function updateVisibility(state) {
      const elements = bodyEl.querySelectorAll("[data-visible-if]");
      elements.forEach((elem) => {
        const cond = elem.dataset.visibleIf;
        const [depKey, op, val] = cond
          .split(/([==!=><]+)/)
          .map((s) => s.trim());
        const depEl = state.inputs.find((e) => e.dataset.modalKey === depKey);
        if (!depEl) {
          elem.style.display = "none";
          return;
        }
        const depVal = depEl.value || depEl.dataset.value || depEl.textContent;
        let show = false;
        switch (op) {
          case "==":
            show = depVal === val;
    ...
    function parseContentArray(contentArr, state) {
      const nodes = [];
      for (const rawItem of contentArr || []) {
        if (!rawItem) continue;
        if (rawItem instanceof Node) {
          nodes.push(rawItem);
          continue;
        }
        const item = rawItem.toString().trim();
        const colon = item.indexOf(":");
        if (colon === -1) {
          continue;
        }
        const type = item.slice(0, colon).trim().toLowerCase();
        ...
        if (type === "row") {
          const groups = [],
            re = /\[([^[]+)\]/g;
          let m;
          while ((m = re.exec(rest)) !== null) groups.push(m[1].trim());
          const childNodes = [];
          for (const g of groups) {
            const firstComma = g.indexOf(",");
            let gType = "",
              payload = "";
            if (firstComma === -1) {
              gType = g.trim().toLowerCase();
              payload = "";
            } else {
              gType = g.slice(0, firstComma).trim().toLowerCase();
              payload = g.slice(firstComma + 1).trim();
            }
            const gOpts = parseOpts(payload);
            let el;
            switch (gType) {
              case "message":
                el = createModalElement("message", {
                  text: gOpts.value || gOpts.text || ""
                });
                break;
              case "input":
                el = createModalElement("input", {
                  key: sanitizeKey(gOpts.placeholder || "input"),
                  inputType: gOpts.type || "text",
                  placeholder: gOpts.placeholder || ""
                });
                break;
              case "textarea":
                el = createModalElement("textarea", {
                  key: sanitizeKey(gOpts.placeholder || "textarea"),
                  placeholder: gOpts.placeholder || ""
                });
                break;
              case "select": {
                const optsList = splitOptions(gOpts.options || payload);
                const options = optsList.map((o) => {
                  if (o.includes("|")) {
                    const [label, value] = o.split("|").map((s) => s.trim());
                    return { label, value };
                  } else {
                    return { label: o, value: o };
                  }
                });
                el = createModalElement("select", {
                  key: sanitizeKey(options[0]?.label || "select"),
                  options
                });
                break;
              }
              case "checkbox":
                el = createModalElement("checkbox", {
                  key: sanitizeKey(gOpts.label || gOpts.value || "cb"),
                  label: gOpts.label || gOpts.placeholder || gOpts.value
                });
                break;
              case "radio":
                el = createModalElement("radio", {
                  key: sanitizeKey(gOpts.label || gOpts.value || "radio"),
                  label: gOpts.label || gOpts.placeholder || gOpts.value
                });
                break;
              case "button":
                el = createModalElement("button", {
                  value: gOpts.value || gOpts.label || "Button"
                });
                break;
              default:
                el = createModalElement(gType, gOpts);
                break;
            }
            if (el.input) state.inputs.push(el.input);
            childNodes.push(el);
          }
          const row = createModalElement("row", {
            children: childNodes,
        ...
        switch (type) {
          ...
          case "input":
            el = createModalElement("input", {
              key: sanitizeKey(opts.placeholder || "input"),
          ...
          case "textarea":
            el = createModalElement("textarea", {
              key: sanitizeKey(opts.placeholder || "textarea"),
              placeholder: opts.placeholder || ""
            });
            break;
          case "select": {
            const optsList = splitOptions(opts.options || rest);
            const options = optsList.map((o) => {
              if (o.includes("|")) {
                const [label, value] = o.split("|").map((s) => s.trim());
                return { label, value };
          ...
          case "checkbox":
            el = createModalElement("checkbox", {
              key: sanitizeKey(opts.label || opts.value || "cb"),
              label: opts.label || opts.placeholder || opts.value
            });
            break;
          case "radio":
            el = createModalElement("radio", {
              key: sanitizeKey(opts.label || opts.value || "radio"),
              label: opts.label || opts.placeholder || opts.value
            });
            break;
          case "button":
            el = createModalElement("button", {
              value: opts.value || opts.label || "Button"
            });
            break;
          default:
            el = createModalElement(type, opts);
            break;
        }
        if (el.input) state.inputs.push(el.input);
        nodes.push(el.el);
    ...
    window.closeModal = function (result = null) {
      if (resolver) {
        try {
          resolver(result);
        } catch (e) {}
      ...
      if (dialog) dialog.removeAttribute("aria-labelledby");
      bodyEl.innerHTML = "";
      footerEl.innerHTML = "";
      titleEl.textContent = "";
      if (openState && openState.cleanupHandlers) {
        for (const fn of openState.cleanupHandlers) {
          try {
            fn();
          } catch (_) {}
        }
    ...
    window.showModal = function showModal(options = {}) {
      ensureModal();
      if (!options.content) {
        const content = [];
        if (options.message) content.push(`message: ${options.message}`);
        if (options.input) {
          if (Array.isArray(options.input))
            for (const i of options.input) content.push(`input: ${i}`);
          else content.push(`input: ${options.input}`);
        }
        Object.keys(options).forEach((k) => {
          if (/^message\\d+$/.test(k)) content.push(`message: ${options[k]}`);
          if (/^input\\d+$/.test(k)) content.push(`input: ${options[k]}`);
        });
        if (content.length) options.content = content;
      }
      return new Promise((resolve) => {
        resolver = resolve;
        openState = { inputs: [], keys: new Set(), cleanupHandlers: [] };
        titleEl.textContent = options.title || "";
        dialog.setAttribute("aria-labelledby", titleEl.id);
        bodyEl.innerHTML = "";
        footerEl.innerHTML = "";
        const contentArr = Array.isArray(options.content)
          ? options.content
          : [];
        const nodes = parseContentArray(contentArr, openState);
        nodes.forEach((n) => bodyEl.appendChild(n));
        setupConditionals(openState);
        ...
        setTimeout(() => {
          bodyEl.querySelectorAll(".modal-row").forEach((row) => {
            ...
            if (buttons.length > 1) {
              let maxBtnWidth = 0;
              buttons.forEach((b) => {
                maxBtnWidth = Math.max(maxBtnWidth, b.offsetWidth);
              });
              buttons.forEach((b) => {
                b.style.width = `${maxBtnWidth}px`;
              });
            }
          });
        }, 30);
        const actions = [];
        if (Array.isArray(options.button)) actions.push(...options.button);
        else if (typeof options.button === "string")
          actions.push(
            ...options.button
              .split(",")
              .map((s) => s.trim())
        ...
        for (const label of actions) {
          ...
          btn.addEventListener("click", () => {
            const values = collectValues(openState);
            closeModal({ action: label, values });
          });
        ...
        setTimeout(() => {
          const first = bodyEl.querySelector(
            "[data-modal-key], input, textarea, select, button"
          );
          if (first && first.focus) first.focus();
        }, 30);
      });
    };
    window.setupModalSystem = function setupModalSystem() {
      ensureModal();
    };
    window.setupModalSystem();
  })();
  window.handleFormat = function (t) {
    const r = preserveSelection(async function () {
      ...
      while (1) {
        const a = [];
        if (errorMsg) a.push("message:" + errorMsg);
        let opts = "";
        if (t === "italic")
          opts = "Choose Format, MarkDown (*text*)|1, MarkUp (<i>text</i>)|2";
        else if (t === "underline")
          opts = "Choose Format, MarkDown (__text__)|1, MarkUp (<u>text</u>)|2";
        else if (t === "bold")
          opts = "Choose Format, MarkDown (**text**)|1, MarkUp (<b>text</b>)|2";
        else if (t === "code")
          opts = "Choose Format, MarkDown (```)|1, MarkUp (<code>)|2";
        a.push("select: " + opts);
        const n = await showModal({
          title: `${t[0].toUpperCase() + t.slice(1)} Text`,
          content: a,
          button: "Cancel, OK"
        });
        if (!n || !n.action || n.action === "Cancel") return;
        const c = String(n.values.choose_format).trim();
        if (c === "" || c === "Choose Format") {
          errorMsg = "Please select a format!";
          continue;
        }
        const sel = v.slice(s, e);
        let f = "";
        if (t === "italic") f = c === "2" ? `<i>${sel}</i>` : `*${sel}*`;
        else if (t === "underline")
          f = c === "2" ? `<u>${sel}</u>` : `__${sel}__`;
        else if (t === "bold") f = c === "2" ? `<b>${sel}</b>` : `**${sel}**`;
        else if (t === "code")
          f = c === "2" ? `<code>${sel}</code>` : `\`\`\`\n${sel}\n\`\`\``;
        noteTextarea.value = v.slice(0, s) + f + v.slice(e);
        updateNoteMetadata();
  ...
  window.handleBulletList = function () {
    const r = preserveSelection(async function () {
      ...
      while (1) {
        ...
        if (errorMsg) a.push("message:" + errorMsg);
        a.push(
          "select: Choose Format, MarkDown (- item)|1, MarkUp (<ul><li>)|2"
        );
        const n = await showModal({
          title: "Bullet List",
          content: a,
  ...
  window.handleNumberedList = function () {
    const r = preserveSelection(async function () {
      ...
      while (1) {
        ...
        if (errorMsg) a.push("message:" + errorMsg);
        a.push(
          "select: Choose Format, MarkDown (1. item)|1, MarkUp (<ol><li>)|2"
        );
        const n = await showModal({
          title: "Numbered List",
          content: a,
    ...
    const r = preserveSelection(async function () {
      ...
      const n = await showModal({
        ...
        content: [
          ...
          "Select: Choose Format, Markdown, MarkUp"
        ],
        button: "Cancel, OK"
      });
      if (!n || !n.action || n.action === "Cancel") return;
      const url = String(n.values.enter_url || "").trim(),
        text = String(n.values.enter_link_text || "").trim(),
        fmt = String(n.values.choose_format || "").trim();
      if (!url || !text || !fmt) return;
      const f =
        fmt === "MarkUp" ? `<a href="${url}">${text}</a>` : `[${text}](${url})`;
      noteTextarea.value = v.slice(0, s) + f + v.slice(e);
      updateNoteMetadata();
  ...
  window.handleInsertImage = function () {
    const r = preserveSelection(async function () {
      ...
      if (!n || !n.action || n.action === "Cancel") return;
      const url = String(n.values.enter_image_url || "").trim(),
        fmt = String(n.values.choose_format || "").trim();
      if (!url || !fmt) return;
      const f =
        fmt === "MarkUp"
          ...
          : `![Image](${url})`;
      noteTextarea.value = s === e ? v + f : v.slice(0, s) + f + v.slice(e);
      updateNoteMetadata();
