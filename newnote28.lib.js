...
window.handleAdd=async()=>{if(!currentNote||!noteTextarea)return;const r=await showModal({header:`<div class="modal-title">Add Text to Lines</div>`,body:`<div style="display:flex;flex-direction:column;gap:10px;"><div><label class="modal-label">Insert text</label><input type="text" id="insertText" class="modal-input" placeholder="Text to insert (use %L for line number, %N for new line)"></div><div><label class="modal-label">Insert position</label><div id="insertPosition" class="custom-dropdown"><div class="custom-dropdown-trigger modal-input" data-options='[{"label":"Start of line","value":"start"},{"label":"End of line","value":"end"},{"label":"Specific column","value":"column"}]'>Start of line</div></div></div><div id="colContainer" style="display:none"><label class="modal-label">Column number</label><input type="number" id="columnNumber" class="modal-input" placeholder="Column number (1-based)"></div></div>`,footer:`<button onclick="closeModal()">Cancel</button><button onclick="handleAddSubmit()" style="background: var(--accent-2); color: #cacaca;" data-skip-validation>Add</button>`,html:true});if(!r||r.action!=="submit")return;const t=modalScope.insertText?modalScope.insertText.value:"",o=modalScope.insertPosition?modalScope.insertPosition.dataset.value:"start",n=modalScope.columnNumber?parseInt(modalScope.columnNumber.value,10)||1:1,e=noteTextarea.value.split("\n");let l="";for(let i=0;i<e.length;i++){let a=e[i],s=t.replace(/%L/g,String(i+1)).replace(/%N/g,"\n");"start"===o?a=s+a:"end"===o?a=a+s:"column"===o&&(a=a.padEnd(Math.max(a.length,n-1)," "),a=a.slice(0,n-1)+s+a.slice(n-1)),l+=a+(i<e.length-1?"\n":"")}noteTextarea.value=l,typeof updateNoteMetadata=="function"&&updateNoteMetadata(),typeof updatecounts=="function"&&updatecounts(),showNotification("Text added successfully!"),closeModal()};document.addEventListener("click",e=>{const t=e.target.closest(".custom-dropdown-trigger");if(!t)return;const o=t.closest(".custom-dropdown");if(!o)return;const n=document.getElementById("colContainer");const l=JSON.parse(t.dataset.options||"[]").find(e=>e.label===t.textContent);n&&(n.style.display=l&&l.value==="column"?"block":"none")});    
