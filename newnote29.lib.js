...
window.handleAdd=async()=>{if(!currentNote||!noteTextarea)return;const r=await showModal({header:`<div class="modal-title">Add Text to Lines</div>`,body:`<div style="display:flex;flex-direction:column;gap:10px"><div style="display:flex;flex-direction:column;gap:6px"><label class="modal-label">Insert text</label><input type="text" id="insertText" class="modal-input" placeholder="Text to insert (use %L for line number, %N for new line)"></div><div style="display:flex;flex-direction:column;gap:6px"><label class="modal-label">Insert position</label><div class="custom-dropdown"><div id="insertPosition" class="custom-dropdown-trigger modal-input" data-options='[{"label":"Start of line","value":"start"},{"label":"End of line","value":"end"},{"label":"Specific column","value":"column"}]'>Start of line</div></div></div><div style="display:flex;flex-direction:column;gap:6px"><label class="modal-label">Column number</label><input type="number" id="columnInput" class="modal-input" min="1" style="display:none" placeholder="Column number (1-based)"></div></div>`,footer:`<button onclick="closeModal()">Cancel</button><button onclick="handleAddSubmit()" style="background: var(--accent-2); color:#cacaca">Add</button>`});if(!r)return;const insert=document.getElementById("insertText").value||"",pos=document.getElementById("insertPosition").dataset.value,col=parseInt(document.getElementById("columnInput").value,10)||1,lines=noteTextarea.value.split("\n");let result="";for(let i=0;i<lines.length;i++){let line=lines[i],rep=insert.replace(/%L/g,String(i+1)).replace(/%N/g,"\n");if(pos==="start")line=rep+line;else if(pos==="end")line=line+rep;else if(pos==="column"){const idx=Math.max(0,col-1);if(line.length<idx)line=line.padEnd(idx," ");line=line.slice(0,idx)+rep+line.slice(idx)}result+=line+(i<lines.length-1?"\n":"")}noteTextarea.value=result;typeof updateNoteMetadata==="function"&&updateNoteMetadata();typeof updatecounts==="function"&&updatecounts();showNotification("Text added successfully!")};window.handleAddInit=function(){const d=document.getElementById("insertPosition"),c=document.getElementById("columnInput");if(!d||!c)return;const opts=JSON.parse(d.dataset.options||"[]");d.addEventListener("click",()=>{const menu=document.createElement("div");menu.className="custom-dropdown-portal-menu active";opts.forEach(o=>{const opt=document.createElement("div");opt.className="custom-dropdown-option";opt.textContent=o.label;opt.dataset.value=o.value;opt.tabIndex=0;opt.addEventListener("click",()=>{d.textContent=o.label;d.dataset.value=o.value;c.style.display=o.value==="column"?"block":"none";menu.remove()});menu.appendChild(opt)});document.body.appendChild(menu);const rect=d.getBoundingClientRect();menu.style.left=rect.left+window.scrollX+"px";menu.style.top=rect.bottom+window.scrollY+"px";menu.style.width=rect.width+"px";const outside=(e)=>{if(!menu.contains(e.target)&&e.target!==d){menu.remove();document.removeEventListener("mousedown",outside)}};document.addEventListener("mousedown",outside)})};window.handleAddInit();    
