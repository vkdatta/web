function initApp() {
  (function () {
    ...
    function evalInModalScope(code) {
      try {
        return new Function('modal', `with(modal) { ${code} }`)(modalScope);
      } catch (error) {
        console.error('Error evaluating modal code:', error);
        return null;
      }
    }
        ...
        footerDiv.querySelectorAll('button').forEach(button => {
          ...
          if (onclickAttr) {
            button.addEventListener('click', () => {
              evalInModalScope(onclickAttr);
            });
            button.removeAttribute('onclick');
          } else if (!button.hasAttribute('onclick')) {
            button.addEventListener('click', () => {
              const values = {};
              Object.keys(modalScope).forEach(id => {
                const element = modalScope[id];
                if (element.tagName === 'INPUT') {
