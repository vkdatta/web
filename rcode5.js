...
(function(){
  ...
  class CompletePrismLanguageLoader {
    ...
    buildPrimaryExtensionMap(){
      for(const [ext,lang] of Object.entries(this.extensionMap)){if(!this.primaryExtensionMap[lang])this.primaryExtensionMap[lang]=ext}
    }
    detectLanguageFromExtension(ext){return this.extensionMap[(ext||'').toLowerCase()]||"none"}
    getLanguageDisplayName(langAttr){const lang=this.detectLanguageFromExtension(langAttr);return this.primaryExtensionMap[lang]||((langAttr||'txt').toLowerCase())}
    async loadLanguage(lang){if(!lang||lang==="none")return Promise.resolve();if(this.loadedLanguages.has(lang))return Promise.resolve();if(this.loadPromises.has(lang))return this.loadPromises.get(lang);const loadPromise=this._loadLanguage(lang);this.loadPromises.set(lang,loadPromise);return loadPromise}
    async _loadLanguage(lang){if(this.loadedLanguages.has(lang)||this.loadingLanguages.has(lang))return;this.loadingLanguages.add(lang);const script=document.createElement('script');script.src=`https://cdnjs.cloudflare.com/ajax/libs/prism/1.29.0/components/prism-${lang}.min.js`;return new Promise((resolve,reject)=>{script.onload=()=>{this.loadedLanguages.add(lang);this.loadingLanguages.delete(lang);resolve()};script.onerror=()=>{this.loadingLanguages.delete(lang);reject(new Error(`Failed to load language: ${lang}`))};document.head.appendChild(script)})}
    isLanguageLoaded(lang){return this.loadedLanguages.has(lang)}
  }
  const languageLoader=new CompletePrismLanguageLoader();
  function fastEscapeHtml(text){const d=document.createElement('div');d.textContent=text;return d.innerHTML}
  function debounce(fn,wait=120){let t;return(...a)=>{clearTimeout(t);t=setTimeout(()=>fn(...a),wait)}}
  async function highlightExistingCodeElement(codeEl){
    ...
    const lineCount=rawText.length?rawText.split('\n').length:0;
    if(lang!=="none"){
      try{
        await languageLoader.loadLanguage(lang);
        if(window.Prism&&languageLoader.isLanguageLoaded(lang)){codeEl.className=`code-content language-${lang}`;Prism.highlightElement(codeEl)}
        else codeEl.innerHTML=fastEscapeHtml(rawText)
      }catch(e){codeEl.innerHTML=fastEscapeHtml(rawText)}
    }else codeEl.innerHTML=fastEscapeHtml(rawText);
    return{displayName,lineCount}
  }
  function createWrapperAndInsert(originalCodeEl,displayName,lineCount){
    const container=document.createElement('div');container.className='code-container';
    const header=document.createElement('div');header.className='code-header';
    const languageBadge=document.createElement('span');languageBadge.className='language-badge';languageBadge.textContent=displayName||'txt';header.appendChild(languageBadge);
    const toolbar=document.createElement('div');toolbar.className='code-toolbar';
    const collapseBtn=document.createElement('span');collapseBtn.className='code-btn collapse-btn';collapseBtn.title='Collapse code block';collapseBtn.innerHTML='<i class="material-icons-round">unfold_less</i>';
    const wrapBtn=document.createElement('span');wrapBtn.className='code-btn wrap-btn';wrapBtn.title='Toggle line wrap';wrapBtn.innerHTML='<i class="material-icons-round">wrap_text</i>';
    const copyBtn=document.createElement('span');copyBtn.className='code-btn copy-btn';copyBtn.title='Copy code';copyBtn.innerHTML='<i class="material-icons-round">content_copy</i>';
    toolbar.appendChild(collapseBtn);toolbar.appendChild(wrapBtn);toolbar.appendChild(copyBtn);header.appendChild(toolbar);container.appendChild(header);
    const pre=document.createElement('pre');pre.className='code-pre';pre.appendChild(originalCodeEl);container.appendChild(pre);
    const hiddenLinesIndicator=document.createElement('div');hiddenLinesIndicator.className='hidden-lines-indicator';hiddenLinesIndicator.innerHTML=`<span>${lineCount} lines hidden</span>`;container.appendChild(hiddenLinesIndicator);
    let isCollapsed=false;let isWrapped=false;
    collapseBtn.addEventListener('click',function(e){e.stopPropagation();isCollapsed=!isCollapsed;container.classList.toggle('collapsed',isCollapsed);if(isCollapsed){collapseBtn.innerHTML='<i class="material-icons-round">unfold_more</i>';collapseBtn.title='Expand code block'}else{collapseBtn.innerHTML='<i class="material-icons-round">unfold_less</i>';collapseBtn.title='Collapse code block'}});
    wrapBtn.addEventListener('click',function(){isWrapped=!isWrapped;pre.classList.toggle('line-wrap',isWrapped);wrapBtn.innerHTML=isWrapped?'<i class=\"material-icons-round\">code</i>':'<i class=\"material-icons-round\">wrap_text</i>';wrapBtn.title=isWrapped?'Unwrap lines':'Wrap lines'});
    copyBtn.addEventListener('click',function(e){e.stopPropagation();const t=(originalCodeEl.textContent||'');navigator.clipboard.writeText(t).then(()=>{const icon=copyBtn.querySelector('i');const orig=icon.innerHTML;icon.innerHTML='check';container.classList.add('copy-success');setTimeout(()=>{icon.innerHTML=orig;container.classList.remove('copy-success')},1500)}).catch(()=>{})});
    return container
  }
  const perElementObservers=new WeakMap();
  function observeCodeTextChanges(codeEl,rehighlightFn){
    if(perElementObservers.has(codeEl))return;
    const debounced=debounce(()=>rehighlightFn(codeEl),120);
    const obs=new MutationObserver((mutations)=>{for(const m of mutations){if(m.type==='characterData'||m.type==='childList'||(m.type==='attributes'&&m.attributeName==='lang')){debounced();break}}});
    obs.observe(codeEl,{characterData:true,subtree:true,childList:true,attributes:true});
    perElementObservers.set(codeEl,obs)
  }
  async function processCodeElement(codeEl){
    if(!codeEl||codeEl.dataset.codeProcessed==='1')return;
    codeEl.dataset.codeProcessed='1';
    try{
      const parent=codeEl.parentNode;
      const preParent=parent&&parent.tagName&&parent.tagName.toLowerCase()==='pre'?parent:null;
      const referenceParent=preParent||parent;
      const nextSibling=referenceParent?referenceParent.nextSibling:null;
      const {displayName,lineCount}=await highlightExistingCodeElement(codeEl);
      const container=createWrapperAndInsert(codeEl,displayName,lineCount);
      if(preParent&&preParent.parentNode){preParent.parentNode.replaceChild(container,preParent)}else if(parent&&parent.parentNode){parent.parentNode.replaceChild(container,parent)}else{document.body.appendChild(container)}
      const updateHiddenLines=()=>{const txt=((codeEl.textContent||'').trim());const lc=txt.length?txt.split('\n').length:0;const indicator=container.querySelector('.hidden-lines-indicator span');if(indicator)indicator.textContent=`${lc} lines hidden`};
      const rehighlight=async(c)=>{try{await highlightExistingCodeElement(c);updateHiddenLines()}catch(e){}};
      observeCodeTextChanges(codeEl,rehighlight)
    }catch(e){
      try{if(codeEl.parentNode)codeEl.parentNode.replaceChild((()=>{const d=document.createElement('div');d.className='code-container';d.innerHTML=`<div class=\"code-header\"><span class=\"language-badge\">txt</span><div class=\"code-toolbar\"><span class=\"code-btn copy-btn\" title=\"Copy code\"><i class=\"material-icons-round\">content_copy</i></span></div></div><pre class=\"code-pre\">${fastEscapeHtml(codeEl.textContent||'')}</pre><div class=\"hidden-lines-indicator\"><span>${(codeEl.textContent||'').trim().split('\n').length} lines hidden</span></div>`;return d})(),codeEl)}catch(e2){}}
  }
    ...
    for(const m of mutations){
      if(m.type==='childList'){
        for(const node of m.addedNodes){
          ...
          if(node.tagName&&node.tagName.toLowerCase()==='code'){processCodeElement(node)}else{
            const codes=node.querySelectorAll&&node.querySelectorAll('code');
            if(codes&&codes.length)codes.forEach(c=>processCodeElement(c))
          }
          if((node.matches&&node.matches('input[data-code-target],textarea[data-code-target]'))||(node.querySelectorAll&&node.querySelectorAll('input[data-code-target],textarea[data-code-target]').length)){
            const inputs=node.matches?([].slice.call(node.matches('input[data-code-target],textarea[data-code-target]')? [node]:[]):[]):[]; // noop to keep flow
          }
        }
      }else if(m.type==='attributes'){
        const t=m.target;
        if((t.tagName==='INPUT'||t.tagName==='TEXTAREA')&&t.dataset&&t.dataset.codeTarget)updateCodeTargetFromInput(t)
      }
    }
  });
  globalObserver.observe(document.documentElement||document.body,{childList:true,subtree:true,attributes:true,attributeFilter:['value']});
  function wireUpInputToCode(inp){if(!inp||inp.__codeWired)return;inp.__codeWired=true;const update=()=>updateCodeTargetFromInput(inp);inp.addEventListener('input',update);const obs=new MutationObserver(()=>update());obs.observe(inp,{attributes:true,attributeFilter:['value']})}
  function updateCodeTargetFromInput(inp){
    const selector=inp.dataset.codeTarget;
    if(!selector)return;
    try{
      const target=document.querySelector(selector);
      if(!target)return;
      if(target.tagName&&target.tagName.toLowerCase()==='code'){target.textContent=inp.value}
      else{
        const codeChild=target.querySelector&&target.querySelector('code');
        if(codeChild)codeChild.textContent=inp.value;
        else target.innerHTML=`<code>${fastEscapeHtml(inp.value)}</code>`
      }
    }catch(e){}
  }
  async function initCodeBlocks(){
    if(!window.Prism){
      await new Promise((resolve,reject)=>{const s=document.createElement('script');s.src='https://cdnjs.cloudflare.com/ajax/libs/prism/1.29.0/prism.min.js';s.onload=resolve;s.onerror=reject;document.head.appendChild(s)})
    }
    const allCodes=Array.from(document.querySelectorAll('code'));
    for(const codeEl of allCodes){
      if(codeEl.closest&&codeEl.closest('.code-container'))continue;
      await processCodeElement(codeEl)
    }
    const inputs=Array.from(document.querySelectorAll('input[data-code-target],textarea[data-code-target]'));
    inputs.forEach(wireUpInputToCode)
  }
