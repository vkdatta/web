...
async function checkPassword(note,callback){const r=await showModal({header:`<div class="modal-title">Enter Password</div>`,body:`<div><label class="modal-label">Please enter the password for this note:</label><input type="password" id="password" class="modal-input"></div>`,footer:`<button onclick="closeModal()">Cancel</button><button onclick="handleCheckPasswordSubmit()" style="background: var(--accent-2); color: #cacaca;">Submit</button>`});if(!r||r.action!=='submit')return;const password=r.password;if(password===note.password){callback();showNotification('Note unlocked!');}else{showNotification('Incorrect password!');const r2=await showModal({header:`<div class="modal-title">Password Recovery</div>`,body:`<div><label class="modal-label">Enter DOB (YYYY-MM-DD) or clear note:</label><input type="text" id="dobInput" class="modal-input"></div>`,footer:`<button onclick="closeModal()">Cancel</button><button onclick="handleRecoverySubmit()" style="background: var(--accent-2); color: #cacaca;">Submit</button>`});if(!r2||r2.action!=='submit')return;const dobInput=r2.dobInput;if(dobInput===dob){callback();showNotification('Note unlocked via DOB!');}else{const r3=await showModal({header:`<div class="modal-title">Clear Note</div>`,body:`<div class="modal-label">Incorrect DOB. Clear note to continue?</div>`,footer:`<button onclick="closeModal()">No</button><button onclick="handleClearConfirm()" style="background: var(--accent-2); color: #cacaca;">Yes</button>`});if(r3&&r3.confirmed){note.content='';note.password='';saveNotes();callback();showNotification('Note cleared and unlocked!');}}}}window.handleCheckPasswordSubmit=function(){closeModal({action:'submit',password:modalScope.password.value});};window.handleRecoverySubmit=function(){closeModal({action:'submit',dobInput:modalScope.dobInput.value});};window.handleClearConfirm=function(){closeModal({confirmed:true});};function checkPasswordRequirement(){const lastCheck=localStorage.getItem('lastPasswordCheck');const now=Date.now();if(!lastCheck||(now-parseInt(lastCheck)>15*60*1000)){notes.forEach(note=>{if(note.password&&note===currentNote){checkPassword(note,()=>{});}});localStorage.setItem('lastPasswordCheck',now);}}window.handlePasswordProtect=async()=>{if(!currentNote){showNotification('No note selected');return;}if(!currentNote.password){let r;if(!dob){r=await showModal({header:`<div class="modal-title">Set Password</div>`,body:`<div><label class="modal-label">Enter password:</label><input type="password" id="password" class="modal-input"></div><div><label class="modal-label">Enter DOB (YYYY-MM-DD) for recovery:</label><input type="text" id="dobInput" class="modal-input"></div>`,footer:`<button onclick="closeModal()">Cancel</button><button onclick="handleSetPasswordDobSubmit()" style="background: var(--accent-2); color: #cacaca;">Set</button>`});if(!r||r.action!=='submit')return;const{password,dobInput}=r;if(password&&dobInput){const r2=await showModal({header:`<div class="modal-title">Security Warning</div>`,body:`<div class="modal-label">Password is stored in local storage and may be accessible to tech-savvy users. Proceed?</div>`,footer:`<button onclick="closeModal()">No</button><button onclick="handleWarningConfirm()" style="background: var(--accent-2); color: #cacaca;">Yes</button>`});if(r2&&r2.confirmed){currentNote.password=password;localStorage.setItem('dob',dobInput);dob=dobInput;saveNotes();showNotification('Password set!');}}}else{r=await showModal({header:`<div class="modal-title">Set Password</div>`,body:`<div><label class="modal-label">Enter password:</label><input type="password" id="password" class="modal-input"></div>`,footer:`<button onclick="closeModal()">Cancel</button><button onclick="handleSetPasswordSubmit()" style="background: var(--accent-2); color: #cacaca;">Set</button>`});if(!r||r.action!=='submit')return;const password=r.password;if(password){const r2=await showModal({header:`<div class="modal-title">Security Warning</div>`,body:`<div class="modal-label">Password is stored in local storage and may be accessible to tech-savvy users. Proceed?</div>`,footer:`<button onclick="closeModal()">No</button><button onclick="handleWarningConfirm()" style="background: var(--accent-2); color: #cacaca;">Yes</button>`});if(r2&&r2.confirmed){currentNote.password=password;saveNotes();showNotification('Password set!');}}}}else{const r=await showModal({header:`<div class="modal-title">Password Options</div>`,body:`<div><label class="modal-label">Remove or change password? Enter "remove" or "change":</label><input type="text" id="action" class="modal-input"></div>`,footer:`<button onclick="closeModal()">Cancel</button><button onclick="handleActionSubmit()" style="background: var(--accent-2); color: #cacaca;">Submit</button>`});if(!r||r.action!=='submit')return;const action=r.action;if(action==='remove'){const r2=await showModal({header:`<div class="modal-title">Remove Password</div>`,body:`<div><label class="modal-label">Enter current password:</label><input type="password" id="password" class="modal-input"></div>`,footer:`<button onclick="closeModal()">Cancel</button><button onclick="handleRemovePasswordSubmit()" style="background: var(--accent-2); color: #cacaca;">Remove</button>`});if(!r2||r2.action!=='submit')return;const password=r2.password;if(password===currentNote.password){currentNote.password='';saveNotes();showNotification('Password removed!');}else{showNotification('Incorrect password!');}}else if(action==='change'){const r2=await showModal({header:`<div class="modal-title">Change Password</div>`,body:`<div><label class="modal-label">Enter current password:</label><input type="password" id="oldPassword" class="modal-input"></div><div><label class="modal-label">Enter new password:</label><input type="password" id="newPassword" class="modal-input"></div>`,footer:`<button onclick="closeModal()">Cancel</button><button onclick="handleChangePasswordSubmit()" style="background: var(--accent-2); color: #cacaca;">Change</button>`});if(!r2||r2.action!=='submit')return;const{oldPassword,newPassword}=r2;if(oldPassword===currentNote.password){currentNote.password=newPassword;saveNotes();showNotification('Password changed!');}else{showNotification('Incorrect current password!');}}}};window.handleSetPasswordDobSubmit=function(){closeModal({action:'submit',password:modalScope.password.value,dobInput:modalScope.dobInput.value});};window.handleSetPasswordSubmit=function(){closeModal({action:'submit',password:modalScope.password.value});};window.handleWarningConfirm=function(){closeModal({confirmed:true});};window.handleActionSubmit=function(){closeModal({action:'submit',action:modalScope.action.value});};window.handleRemovePasswordSubmit=function(){closeModal({action:'submit',password:modalScope.password.value});};window.handleChangePasswordSubmit=function(){closeModal({action:'submit',oldPassword:modalScope.oldPassword.value,newPassword:modalScope.newPassword.value});};
...
window.saveNotes = () => localStorage.setItem('notes', JSON.stringify(notes));

window.updateNoteMetadata=function(){if(!window.currentNote||!window.noteTextarea)return;window.undoStack.push({content:window.currentNote.content,selectionStart:window.noteTextarea.selectionStart,selectionEnd:window.noteTextarea.selectionEnd});window.redoStack=[];window.currentNote.content=window.noteTextarea.value;window.currentNote.lastEdited=new Date().toISOString();saveNotes();updateDocumentInfo()};

window.updateDocumentInfo=function(){if(!window.currentNote)return document.getElementById("infoName").textContent="-",document.getElementById("infoStats").textContent="{0 | 0 | 0} / 0m",void 0;var e=window.currentNote,t=e.title,n=e.content,o=e.extension,l=n.length,a=n.replace(/\s/g,"").length,i=n.trim().split(/\s+/).filter(w=>w).length,r=Math.ceil(i/200),c=r<60?r+"m":(r/60).toFixed(1)+"h";document.getElementById("infoName").textContent=t+(o?"."+o:""),document.getElementById("infoDexLabs").textContent="Dex Labs",document.getElementById("infoStats").textContent=`{${a} | ${l} | ${i}} / ${c}`};

window.applyFontSize=function(){noteTextarea.style.fontSize=`${fontSize}px`;localStorage.setItem('fontSize',fontSize)},window.increaseFontSize=function(){fontSize=Math.min(fontSize+2,42);window.applyFontSize();showNotification(`Font size increased to ${fontSize}px`)},window.decreaseFontSize=function(){fontSize=Math.max(fontSize-2,10);window.applyFontSize();showNotification(`Font size decreased to ${fontSize}px`)};

window.populateNoteList=function(){if(!noteList)return;noteList.innerHTML='';notes.slice(0,Math.max(1,visibleNotes)).forEach(n=>{const t=document.createElement('li');t.className='note-item';t.dataset.id=n.id;t.innerHTML=`<span class="note-title">${n.title}</span><span class="note-badge">${n.content.length} chars | .${n.extension}</span>`;t.onclick=()=>{showNoteApp(n.id)};if(currentNote&&currentNote.id===n.id)t.classList.add('selected');noteList.appendChild(t);});};  

window.openNote=function(e){const t=notes.find(n=>n.id===e);if(!t)return;if(t.password)checkPassword(t,()=>{currentNote=t;noteTextarea.value=t.content;updateDocumentInfo();document.querySelectorAll(".note-item").forEach(e=>e.classList.remove("selected"));const e=document.querySelector(`.note-item[data-id="${t.id}"]`);e&&e.classList.add("selected")});else{currentNote=t;noteTextarea.value=t.content;updateDocumentInfo();document.querySelectorAll(".note-item").forEach(e=>e.classList.remove("selected"));const e=document.querySelector(`.note-item[data-id="${t.id}"]`);e&&e.classList.add("selected")}};

window.getNextEmptyNote=function(){return notes.find(e=>!e.content)};  
  
window.showNotification=function(e){if(window.notification){window.notification.textContent=e;window.notification.classList.add('show');setTimeout(()=>window.notification.classList.remove('show'),3e3)}};

window.handleNewDocument=()=>{if(visibleNotes>=maxNotes){showNotification("Max notes limit reached! Clear a note to continue.");return}const e=getNextEmptyNote();if(e){visibleNotes=1;updateNoteVisibility();openNote(e.id);showNotification("New document created!")}else{const t={id:Date.now().toString(),title:`note${notes.length+1}.txt`,content:"",extension:"txt",lastEdited:new Date().toISOString(),password:""};notes.push(t);visibleNotes=1;updateNoteVisibility();openNote(t.id);showNotification("New document created!")}};
  
window.handleOpenFile=function(){const e=getNextEmptyNote();if(!e){showNotification("No empty notes available! Clear a note to continue.");return}const t=document.createElement("input");t.type="file";t.accept=".txt,.md,.csv,.json,.xml,.yml,.yaml,.js,.ts,.jsx,.tsx,.html,.css,.py,.java,.c,.cpp,.h,.go,.rb,.php,.rs,.swift,.sh,.bat,Dockerfile,Makefile,.env,.ini,.toml,.conf,.log,.dockerignore";t.onchange=function(o){const n=o.target.files[0];if(n){const r=new FileReader;r.onload=function(o){let x=n.name.split("."),a=x.pop().toLowerCase(),base=x.join(".");e.title=base;e.content=o.target.result;e.extension=a;e.lastEdited=new Date().toISOString();visibleNotes=1;updateNoteVisibility();openNote(e.id);showNotification("File opened!")},r.readAsText(n)}};t.click()};
...
window.showHomepage=function(){if(homepage&&noteAppContainer&&diffCheckerContainer){homepage.style.display="flex";noteAppContainer.style.display="none";diffCheckerContainer.style.display="none";topbar.style.display="none";isHomepage=true;currentApp="home";history.pushState({page:"home"},"","/")}};
