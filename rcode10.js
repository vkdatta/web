...
class L{
  ...
  async load(lang){
    ...
    // Handle dependencies
    const deps = this.dependencies[lang] || [];
    const depPromises = deps.map(dep => this.load(dep));
...
function makeWrapper(codeEl,display,lines){
  ...
  const collapse=document.createElement('span');collapse.className='code-btn collapse-btn';collapse.type='button';collapse.title='Collapse code block';collapse.innerHTML='<i class=\"material-icons-round\">unfold_less</i>';
  const wrap=document.createElement('span');wrap.className='code-btn wrap-btn';wrap.type='button';wrap.title='Toggle line wrap';wrap.innerHTML='<i class=\"material-icons-round\">wrap_text</i>';
  const copy=document.createElement('span');copy.className='code-btn copy-btn';copy.type='button';copy.title='Copy code';copy.innerHTML='<i class=\"material-icons-round\">content_copy</i>';
  ...
  let c=false,w=false;
  collapse.addEventListener('click',function(e){e.stopPropagation();c=!c;container.classList.toggle('collapsed',c);collapse.innerHTML=c?'<i class=\"material-icons-round\">unfold_more</i>':'<i class=\"material-icons-round\">unfold_less</i>';collapse.title=c?'Expand code block':'Collapse code block'});
  ...
  copy.addEventListener('click',function(e){e.stopPropagation();const t=(codeEl.textContent||'');navigator.clipboard.writeText(t).then(()=>{const ic=copy.querySelector('i');const o=ic.innerHTML;ic.innerHTML='check';container.classList.add('copy-success');setTimeout(()=>{ic.innerHTML=o;container.classList.remove('copy-success')},2000)}).catch(()=>{})});
  return container;
}
...
async function process(codeEl){
  ...
  try{
    ...
    try{const fallback=document.createElement('div');fallback.className='code-container';fallback.innerHTML=`<div class=\"code-header\"><span class=\"language-badge\">txt</span><div class=\"code-toolbar\"><span class=\"code-btn collapse-btn\" type=\"button\" title=\"Collapse code block\"><i class=\"material-icons-round\">unfold_less</i></span><span class=\"code-btn wrap-btn\" type=\"button\" title=\"Toggle line wrap\"><i class=\"material-icons-round\">wrap_text</i></span><span class=\"code-btn copy-btn\" type=\"button\" title=\"Copy code\"><i class=\"material-icons-round\">content_copy</i></span></div></div><pre class=\"code-pre\">${esc((codeEl.textContent||''))}</pre><div class=\"hidden-lines-indicator\"><span>${(codeEl.textContent||'').trim().split('\n').length} lines hidden</span></div>`;if(codeEl.parentNode)codeEl.parentNode.replaceChild(fallback,codeEl)}catch(e2){}}
}
