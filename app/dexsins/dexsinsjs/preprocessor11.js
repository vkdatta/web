(function () {
  ...
  const attemptedSrcs = new Set();            // src attributes we attempted
  const requestLog = new Map();               // resolvedUrl -> { status, error }
  let validationDone = false;

  // ---------- PATCH XMLHttpRequest (logs all sync requests) ----------
  (function patchXHR() {
  ...
  // ---------- UTILITIES ----------
  function resolveURL(src, base = document.baseURI || location.href) {
    try {
  ...
  class DextoolsImport extends HTMLElement {
    connectedCallback() {
      ...
      // Record that we attempted this import BEFORE any removal
      attemptedSrcs.add(src);
  ...
  // ---------- VALIDATION & ERROR NUKE ----------
  function validateImports() {
    if (validationDone) return;
    validationDone = true;

    // Gather all missing imports
    const missing = [];
    for (const src of attemptedSrcs) {
      const resolved = resolveURL(src);
    ...
    // Clear log to prevent memory leaks
    requestLog.clear();
...
<div class="card">
  ...
  // ---------- TRIGGER VALIDATION AFTER DOM PARSING ----------
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => setTimeout(validateImports, 0));
  } else {
    setTimeout(validateImports, 0);
  }

  // Optional: observe for dynamically added imports (if needed)
  const observer = new MutationObserver((mutations) => {
    // Only care if new <dextools-import> nodes appear
    for (const mut of mutations) {
      for (const node of mut.addedNodes) {
        if (node.nodeType === Node.ELEMENT_NODE && node.matches(TAG)) {
          // Already handled in connectedCallback tracking
        }
      }
    }
  });
  ...
  // Warn about deprecated sync XHR in dev console
  console.warn(LOG, 'Synchronous XMLHttpRequest is deprecated and may block the UI. Consider migrating to async fetch() in the future.');
})();
