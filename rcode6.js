...
(function(){
  ...
  class L{constructor(){this.loaded=new Set(["markup","css","clike","javascript"]);this.loading=new Set();this.promises=new Map();this.ext={html:"markup",htm:"markup",xml:"markup",svg:"markup",css:"css",js:"javascript",mjs:"javascript",cjs:"javascript",txt:"none"};this.primary={};for(const e in this.ext)if(!this.primary[this.ext[e]])this.primary[this.ext[e]]=e}detect(e){return this.ext[(e||'').toLowerCase()]||"none"}displayName(a){const l=this.detect(a);return this.primary[l]||((a||'txt').toLowerCase())}async load(lang){if(!lang||lang==="none")return; if(this.loaded.has(lang))return; if(this.promises.has(lang))return this.promises.get(lang); const p=this._load(lang);this.promises.set(lang,p);return p}async _load(lang){if(this.loaded.has(lang)||this.loading.has(lang))return;this.loading.add(lang);const sc=document.createElement('script');sc.src=`https://cdnjs.cloudflare.com/ajax/libs/prism/1.29.0/components/prism-${lang}.min.js`;return new Promise((res,rej)=>{sc.onload=()=>{this.loaded.add(lang);this.loading.delete(lang);res()};sc.onerror=()=>{this.loading.delete(lang);rej(new Error('load failed'))};document.head.appendChild(sc)})}isLoaded(lang){return this.loaded.has(lang)}}
  const loader=new L();
  function e(x){const d=document.createElement('div');d.textContent=x;return d.innerHTML}
  function debounce(fn,t=120){let h;return(...a)=>{clearTimeout(h);h=setTimeout(()=>fn(...a),t)}}
  async function highlightExisting(codeEl){
    const langAttr=codeEl.getAttribute('lang')||'txt';const lang=loader.detect(langAttr);const display=loader.displayName(langAttr);const raw=((codeEl.textContent||'').replace(/\r\n/g,'\n')).trim();codeEl.textContent=raw;const lines=raw?raw.split('\n').length:0;if(lang!=="none"){try{await loader.load(lang);if(window.Prism&&loader.isLoaded(lang)){codeEl.classList.add('code-content',`language-${lang}`);Prism.highlightElement(codeEl)}else codeEl.innerHTML=e(raw)}catch(err){codeEl.innerHTML=e(raw)}}else codeEl.innerHTML=e(raw);return{display,lines}}
  function buildWrapper(codeEl,display,lines){
  ...
  async function process(codeEl){
    ...
    try{
      const {display,lines}=await highlightExisting(codeEl);
      const wrapper=buildWrapper(codeEl,display,lines);
      ...
      const rehighlight=async(el)=>{try{await highlightExisting(el);updateHidden()}catch(e){}};
      observeCode(codeEl,rehighlight)
    }catch(e){
      try{const fallback=document.createElement('div');fallback.className='code-container';fallback.innerHTML=`<div class="code-header"><span class="language-badge">txt</span><div class="code-toolbar"><button class="code-btn copy-btn" type="button" title="Copy code"><i class="material-icons-round">content_copy</i></button></div></div><pre class="code-pre">${e((codeEl.textContent||''))}</pre><div class="hidden-lines-indicator"><span>${(codeEl.textContent||'').trim().split('\n').length} lines hidden</span></div>`;if(codeEl.parentNode)codeEl.parentNode.replaceChild(fallback,codeEl)}catch(e2){}}
  }
  const go=new MutationObserver((ms)=>{for(const m of ms){if(m.type==='childList'){for(const n of m.addedNodes){if(!n||n.nodeType!==1)continue;if(n.tagName&&n.tagName.toLowerCase()==='code'){process(n)}else{const cs=n.querySelectorAll&&n.querySelectorAll('code');if(cs&&cs.length)cs.forEach(c=>process(c))}}}else if(m.type==='attributes'){const t=m.target;if((t.tagName==='INPUT'||t.tagName==='TEXTAREA')&&t.dataset&&t.dataset.codeTarget)updateFromInput(t)}}});
  go.observe(document.documentElement||document.body,{childList:true,subtree:true,attributes:true,attributeFilter:['value']});
  ...
  function updateFromInput(inp){const sel=inp.dataset.codeTarget;if(!sel)return;try{const t=document.querySelector(sel);if(!t)return;if(t.tagName&&t.tagName.toLowerCase()==='code'){t.textContent=inp.value}else{const c=t.querySelector&&t.querySelector('code');if(c)c.textContent=inp.value;else t.innerHTML=`<code>${e(inp.value)}</code>`}}catch(e){}}
  async function init(){
    if(!window.Prism)await new Promise((r,j)=>{const sc=document.createElement('script');sc.src='https://cdnjs.cloudflare.com/ajax/libs/prism/1.29.0/prism.min.js';sc.onload=r;sc.onerror=j;document.head.appendChild(sc)});
