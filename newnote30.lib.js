...
window.handleAdd = async () => { if (!currentNote || !noteTextarea) return; const r = await showModal({ header: `<div class="modal-title">Add Text to Lines</div>`, body: `<style>.modal-btn.active { background: var(--accent-2); color: #cacaca; }</style><div style="display:flex;flex-direction:column;gap:10px;"><div><label class="modal-label">Insert text</label><input type="text" id="insertText" class="modal-input" placeholder="Text to insert (use %L for line number, %N for new line)" data-skip-validation></div><div><label class="modal-label">Insert position</label><div style="display:flex;gap:8px;"><button type="button" id="startMode" class="modal-btn active" data-value="start">Start of line</button><button type="button" id="endMode" class="modal-btn" data-value="end">End of line</button><button type="button" id="columnMode" class="modal-btn" data-value="column">Specific column</button></div></div><div id="colContainer" style="display:none"><label class="modal-label">Column number</label><input type="number" id="columnNumber" class="modal-input" placeholder="Column number (1-based)"></div></div>`, footer: `<button onclick="closeModal()">Cancel</button><button onclick="handleAddSubmit()" style="background: var(--accent-2); color: #cacaca;">Add</button>`, html: true }); if (!r || r.action !== "submit") return; const { insertText: t, insertPosition: o, columnNumber } = r; const n = parseInt(columnNumber, 10) || 1; const e = noteTextarea.value.split("\n"); let l = ""; for (let i = 0; i < e.length; i++) { let a = e[i], s = t.replace(/%L/g, String(i + 1)).replace(/%N/g, "\n"); if ("start" === o) a = s + a; else if ("end" === o) a = a + s; else if ("column" === o) { a = a.padEnd(Math.max(a.length, n - 1), " "); a = a.slice(0, n - 1) + s + a.slice(n - 1); } l += a + (i < e.length - 1 ? "\n" : ""); } noteTextarea.value = l; typeof updateNoteMetadata == "function" && updateNoteMetadata(); typeof updatecounts == "function" && updatecounts(); showNotification("Text added successfully!"); }; window.handleAddSubmit = function () { const insertText = modalScope.insertText ? modalScope.insertText.value : ""; const activeButton = document.querySelector('.modal-btn.active'); const insertPosition = activeButton ? activeButton.dataset.value : "start"; const columnNumber = modalScope.columnNumber ? modalScope.columnNumber.value : ""; closeModal({ action: 'submit', insertText, insertPosition, columnNumber }); }; document.addEventListener("click", e => { if (!modalScope) return; const id = e.target.id; if (id === 'startMode' || id === 'endMode' || id === 'columnMode') { ['startMode', 'endMode', 'columnMode'].forEach(i => modalScope[i] && modalScope[i].classList.remove('active')); e.target.classList.add('active'); const colContainer = document.getElementById("colContainer"); colContainer.style.display = (id === 'columnMode') ? 'block' : 'none'; } });    
