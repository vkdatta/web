...
document.addEventListener("click",e=>{if(!modalScope)return;const positionElem=modalScope.insertPosition;if(!positionElem)return;const value=positionElem.dataset.value||'start';const colContainer=document.getElementById("colContainer");if(colContainer){colContainer.style.display=(value==='column')?'block':'none';}});    
