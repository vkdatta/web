...
window.handleAdd=async()=>{if(!currentNote||!noteTextarea)return;const e=await showModal({header:`<div class="modal-title">Add Text to Lines</div>`,body:`<div style="display:flex;flex-direction:column;gap:10px;"><div style="display:flex;flex-direction:column;gap:6px;"><label class="modal-label">Insert text</label><input type="text" id="insertText" class="modal-input" placeholder="Text to insert (use %L for line number, %N for new line)"></div><div style="display:flex;flex-direction:column;gap:6px;"><label class="modal-label">Insert position</label><div id="insertPosition" class="custom-dropdown-trigger modal-input" data-options='[{"label":"Start of line","value":"start"},{"label":"End of line","value":"end"},{"label":"Specific column","value":"column"}]'>Start of line</div></div><div style="display:flex;flex-direction:column;gap:6px;"><label class="modal-label">Column number</label><input type="number" id="columnNumber" class="modal-input" placeholder="Column number (1-based)" style="display:none"></div></div>`,footer:`<button onclick="closeModal()">Cancel</button><button onclick="handleAddSubmit()" style="background: var(--accent-2); color: #cacaca;">Add</button>`,html:!0});if(!e||e.action!=="submit")return;const t=modalScope.insertPosition,n=modalScope.columnNumber;t&&n&&t.addEventListener("click",()=>{setTimeout(()=>{n.style.display=t.dataset.value==="column"?"block":"none"},50)})};window.handleAddSubmit=()=>{const e=modalScope.insertText?modalScope.insertText.value:"",t=modalScope.insertPosition?modalScope.insertPosition.dataset.value:"start",n=parseInt(modalScope.columnNumber?modalScope.columnNumber.value:1,10)||1,r=noteTextarea.value.split("\n");let l="";for(let o=0;o<r.length;o++){let i=r[o],d=e.replace(/%L/g,String(o+1)).replace(/%N/g,"\n");if(t==="start")i=d+i;else if(t==="end")i=i+d;else if(t==="column"){const c=Math.max(0,n-1);i.length<c&&(i=i.padEnd(c," ")),i=i.slice(0,c)+d+i.slice(c)}l+=i+(o<r.length-1?"\n":"")}noteTextarea.value=l,typeof updateNoteMetadata=="function"&&updateNoteMetadata(),typeof updatecounts=="function"&&updatecounts(),showNotification("Text added successfully!"),closeModal()};    
