...
window.handleAdd=async()=>{if(!currentNote||!noteTextarea)return;const r=await showModal({header:`<div class="modal-title">Add Text to Lines</div>`,body:`<div style="display:flex;flex-direction:column;gap:10px;"><div style="display:flex;flex-direction:column;gap:6px;"><label class="modal-label">Insert text</label><input type="text" id="insertText" placeholder="Text to insert (use %L for line number, %N for new line)" class="modal-input"></div><div style="display:flex;flex-direction:column;gap:6px;"><label class="modal-label">Insert position</label><div id="insertPosition" class="custom-dropdown-trigger modal-input" data-options='[{"label":"Start of line","value":"start"},{"label":"End of line","value":"end"},{"label":"Specific column","value":"column"}]'>Start of line</div></div><div style="display:flex;flex-direction:column;gap:6px;"><label class="modal-label">Column number</label><input type="number" id="columnNumber" placeholder="Column number (1-based)" class="modal-input" style="display:none"></div></div>`,footer:`<button onclick="closeModal()">Cancel</button><button onclick="handleAddSubmit()" style="background: var(--accent-2); color: #cacaca;">Add</button>`});if(!r||r.action!=="submit")return};window.handleAddSubmit=function(){const insert=modalScope.insertText?modalScope.insertText.value:"",pos=modalScope.insertPosition?modalScope.insertPosition.dataset.value:"start",col=parseInt(modalScope.columnNumber?modalScope.columnNumber.value:1,10)||1;const lines=noteTextarea.value.split("\n");let result="";for(let i=0;i<lines.length;i++){let line=lines[i],replacement=insert.replace(/%L/g,String(i+1)).replace(/%N/g,"\n");if(pos==="start")line=replacement+line;else if(pos==="end")line=line+replacement;else if(pos==="column"){const idx=Math.max(0,col-1);if(line.length<idx)line=line.padEnd(idx," ");line=line.slice(0,idx)+replacement+line.slice(idx)}result+=line+(i<lines.length-1?"\n":"")}noteTextarea.value=result;if(typeof updateNoteMetadata==="function")updateNoteMetadata();if(typeof updatecounts==="function")updatecounts();showNotification("Text added successfully!");closeModal()};document.addEventListener("DOMContentLoaded",()=>{const posTrigger=document.getElementById("insertPosition"),colInput=document.getElementById("columnNumber");if(posTrigger&&colInput)posTrigger.addEventListener("change",()=>{colInput.style.display=posTrigger.dataset.value==="column"?"block":"none"})});
