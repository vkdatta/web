...
window.handleAdd=async()=>{if(!currentNote||!noteTextarea)return;const r=await showModal({header:`<div class="modal-title">Add Text to Lines</div>`,body:`<div style="display:flex;flex-direction:column;gap:10px;"><div><label class="modal-label">Insert text</label><input type="text" id="insertText" class="modal-input" placeholder="Text to insert (use %L for line number, %N for new line)" data-skip-validation></div><div><label class="modal-label">Insert position</label><div class="custom-dropdown"><div id="insertPosition" class="custom-dropdown-trigger modal-input" data-options='[{"label":"Start of line","value":"start"},{"label":"End of line","value":"end"},{"label":"Specific column","value":"column"}]' data-value="start">Start of line</div></div></div><div id="colContainer" style="display:none"><label class="modal-label">Column number</label><input type="number" id="columnNumber" class="modal-input" placeholder="Column number (1-based)"></div></div>`,footer:`<button onclick="closeModal()">Cancel</button><button onclick="handleAddSubmit()" style="background: var(--accent-2); color: #cacaca;">Add</button>`,html:true});if(!r||r.action!=="submit")return;const{insertText:t,insertPosition:o,columnNumber}=r,n=parseInt(columnNumber,10)||1,e=noteTextarea.value.split("\n");let l="";for(let i=0;i<e.length;i++){let a=e[i],s=t.replace(/%L/g,String(i+1)).replace(/%N/g,"\n");if(o==="start")a=s+a;else if(o==="end")a=a+s;else if(o==="column"){const idx=Math.max(0,n-1);if(a.length<idx)a=a.padEnd(idx," ");a=a.slice(0,idx)+s+a.slice(idx);}l+=a+(i<e.length-1?"\n":"")}noteTextarea.value=l;typeof updateNoteMetadata=="function"&&updateNoteMetadata();typeof updatecounts=="function"&&updatecounts();showNotification("Text added successfully!");};window.handleAddSubmit=function(){const insertText=modalScope.insertText?modalScope.insertText.value:"",insertPosition=modalScope.insertPosition?modalScope.insertPosition.dataset.value:"start",columnNumber=modalScope.columnNumber?modalScope.columnNumber.value:"";closeModal({action:"submit",insertText,insertPosition,columnNumber})};document.addEventListener("click",e=>{if(!modalScope)return;const positionElem=modalScope.insertPosition;if(!positionElem)return;const colContainer=document.getElementById("colContainer");if(colContainer){colContainer.style.display=(positionElem.dataset.value==="column")?"block":"none"}});    
